...
function formatJSON(jsonCodeBlock) {   
    ...
    for (var i = 0, elementCount = jsonCodeBlock.childCodeBlockElements.length; i < elementCount; i++) {
        ...
        var parCdBlkChldElm = currentElement.parentCodeBlock.parentCodeBlock;
        if(parCdBlkChldElm !== null){
            
            var curHighestLevel = [];
            for (var loop = 0; loop < parCdBlkChldElm.length; loop++) {
                curHighestLevel.push(parCdBlkChldElm[loop].keyLength)
            }
            var curGrtLeg = Math.max.apply(Math,curHighestLevel);// Find the largest value/Level

            
            if(parCdBlkChldElm.keyLength < curLrgKeyLength){
                //addSpace = '--->';
            }
        }
