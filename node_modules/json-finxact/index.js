...
function formatJSON(jsonCodeBlock) {   
    ...
    for (var i = 0, elementCount = jsonCodeBlock.childCodeBlockElements.length; i < elementCount; i++) {
        ...
        var curLrgKeyLength = 20;

        if(currentLevel > 1){
            var indSpc = 0;
            var addSpc = 0;
            while (indSpc < (currentLevel-1)) {
                addSpc += 4;
                indSpc++;
            }
            curLrgKeyLength = curLrgKeyLength + addSpc;
        }
        ...
        var leftSpace = '';
        for (let index = 0; index < lengthOfPadding; index++) {// Loop over padding length add space for each count
            leftSpace += ' '; 
        }

        //Bracket spacing
        var lrgKeyLength = Math.max.apply(Math,highestLevelCurrent);// Find the largest value/Level
        var bracketPadding = Math.abs(curLrgKeyLength - lrgKeyLength);
        console.log(currentElement.key, bracketPadding);


        var bracketLeft = '';
        for (let j = 0; j < bracketPadding; j++) {// Loop over padding length add space for each count
            bracketLeft += ' '; 
        }

        /*
        var bracketLeft = '';
        for (let index = 0; index < (lengthOfPadding-2); index++) {// Loop over padding length add space for each count
            bracketLeft += ' '; 
        }
        */
        ...
        if(currentElement.key){// Check if key is set
            generateJSONString += leftSpace + '"' + currentElement.key + '": ';
        }
        ...
        if(currentElement.codeBlockValue !== null && currentElement.codeBlockValue.childCodeBlockElements.length > 0){
            ...
            if(currentElement.blockType == 'Object'){
                ...
                if(!currentElement.key){
                    generateJSONString += bracketLeft;
                }else{
