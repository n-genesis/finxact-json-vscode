...
function deepLook(jsonCodeBlock,paddingSpace) {   
    ...
    for (var i = 0, length = jsonCodeBlock.childCodeBlockElements.length; i < length; i++) {
        ...
        var currentColonPosition = (currentElement.keyLength);// Adding 3 for quote and colon
        ...
        if(currentElement.key){// Check if key is set
            if(currentElement.key.search(/^[0-9]*$/) == -1){
        ...
        if(currentElement.codeBlockValue !== null && currentElement.codeBlockValue.childCodeBlockElements.length > 0){
            ...
            if(currentElement.blockType == 'array'){// Is array block?
                ...
                /** Start Old formatting code */
                if(currentElement.key.search(/^[0-9]*$/) !== -1){// Looking for array with on keyname
                    generateJSONString += '[ ';
                }else {// Check if currentElement is certain key
                    if(currentElement.isAllOf || currentElement.isChoices
                    || currentElement.isForeignKeys || currentElement.isSerialize
                    || currentElement.isComputeds || currentElement.isEnum
                    || currentElement.isContextValues || currentElement.isIndexes
                    || currentElement.isOneOf){
                        generateJSONString += '[\n';
                    }else{
                        generateJSONString += '[ ';
                    }
