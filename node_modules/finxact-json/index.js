...
class codeBlock{
    ...
    //static get childLevels() { return _childLevels; }
    //static set childLevels(value) { _childLevels = value; }
}
...
class JSONConstructor {
        ...
        for (var property in object) {
            if (object.hasOwnProperty(property)) {
                ...
                if(newCodeBlockElement){
                    newCodeBlockElement.childLevels.level += 1;            
    ...
    createCodeBlock(useCodeBlock,currentCodeBlockElement){
        ...
        newCodeBlock.childLevels = new propertyLevel('codeBlock');
        newCodeBlock.childLevels.level += 1;
...
class formatOutput {
    constructor(jsonCodeBlock,specialObj) {
        ...
        this._specialObj = specialObj || false;
    }
    ...
    build(){
        for (var i = 0, elementCount = this.jsonCodeBlock.childCodeBlockElements.length; i < elementCount; i++) {
            ...
            if(setStartColumn > 24 && currentElement.key){
               //currentLevel = (currentLevel+1);
            }
            
            if(this._specialObj == true){
            ...
            if(currentElement.codeBlockValue !== null && currentElement.codeBlockValue.childCodeBlockElements.length > 0){
                ...
                if(currentElement.blockType == 'Array'){
                    ...
                    if(isKeyname(currentElementProp,this.specialFormatKeys)){
                        if(currentElement.key){
                            ...
                            //''.padEnd((currentColonPosition-1));
                            this.generateJSONString += ''.padEnd(currentLevel);
                        }
                ...
                if(currentElement.blockType == 'Object'){
                    ...
                    if(!currentElement.key){
                        ...
                        this._specialObj = true;
                    }
    
                    this.generateJSONString += '{\n'; // Open Bracket

                    var formatJSON = new formatOutput(currentElement.codeBlockValue,this._specialObj);
                    this.generateJSONString += formatJSON.build();

                    //Space for CLOSING object with no keyname
                    if(!currentElement.key){
                        this.generateJSONString += ''.padEnd(setStartColumn);// Special formatting
                        this._specialObj = false;
                    }else{
...
module.exports = {codeBlock, codeBlockElements, propertyLevel, JSONConstructor, formatOutput, formatStr};